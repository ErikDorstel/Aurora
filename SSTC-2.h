...
// call "setSSTC2Tone(freqA,freqB,ratioA,ratioB);" to set parameters for async mode
...
void setSSTC2Sweep(int freq1, int freq2, int duration, int ratio) {
  if (freq1<freq2 & duration>0) { for (int freq=freq1;freq<=freq2;freq+=(freq2-freq1)*20/duration) { setSSTC2Tone(freq,0,ratio,0); delay(20); } setSSTC2Tone(0,0,0,0); }
  if (freq1>freq2 & duration>0) { for (int freq=freq1;freq>=freq2;freq-=(freq1-freq2)*20/duration) { setSSTC2Tone(freq,0,ratio,0); delay(20); } setSSTC2Tone(0,0,0,0); } }
